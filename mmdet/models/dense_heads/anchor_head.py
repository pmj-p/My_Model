...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def get_anchors(self, featmap_sizes, img_metas, device='cuda'):
        ...
        """
        num_imgs = len(img_metas) # num_imgs = 2

        # since feature map sizes of all images are the same, we only compute
        # anchors for one time
        multi_level_anchors = self.prior_generator.grid_priors(
            featmap_sizes, device=device)
        # # 0[136800, 4] 1[34200, 4] 2[8500,4] 3[2223, 4] 4[630, 4]
    ...
    def _get_targets_single(self,
        ...
        """
        inside_flags = anchor_inside_flags(flat_anchors, valid_flags,   # [182403, 4]  [182403]
                                           img_meta['img_shape'][:2],
                                           self.train_cfg.allowed_border)
        if not inside_flags.any():
            return (None, ) * 7
        # assign gt and sample anchors
        anchors = flat_anchors[inside_flags, :] # [182403, 4]
        # 分配正负样本
        ...
        if len(pos_inds) > 0:
            if not self.reg_decoded_bbox:
                pos_bbox_targets = self.bbox_coder.encode(  # 编码过程
    ...
    def get_targets(self,
        ...
        if gt_bboxes_ignore_list is None:
            gt_bboxes_ignore_list = [None for _ in range(num_imgs)] # [None, None]
    ...
    def loss(self,
        ...
        Returns:
            dict[str, Tensor]: A dictionary of loss components.
        """
        featmap_sizes = [featmap.size()[-2:] for featmap in cls_scores]  # 获得5层特征图的高宽
        assert len(featmap_sizes) == self.prior_generator.num_levels

        device = cls_scores[0].device
        # 生成所有anchor和valid_flag_list
        anchor_list, valid_flag_list = self.get_anchors(
            featmap_sizes, img_metas, device=device)
        label_channels = self.cls_out_channels if self.use_sigmoid_cls else 1 # use_sigmoid_cls =true label_channels=80
