...
class SparseRoIHead(CascadeRoIHead):
    ...
    def _bbox_forward(self, stage, x, rois, object_feats, img_metas):
        ...
        bbox_feats = bbox_roi_extractor(x[:bbox_roi_extractor.num_inputs], # [200 256 7 7] 两张图两百个框的roi区域
                                        rois)
        cls_score, bbox_pred, object_feats, attn_feats = bbox_head(
            bbox_feats, object_feats)
        proposal_list = self.bbox_head[stage].refine_bboxes(  # 细化bbox_pred
            rois,
            rois.new_zeros(len(rois)),  # dummy arg
            bbox_pred.view(-1, bbox_pred.size(-1)),
            [rois.new_zeros(object_feats.size(1)) for _ in range(num_imgs)],
            img_metas)
        bbox_results = dict(  # 将结果整理成一个字典
            cls_score=cls_score,
            decode_bbox_pred=torch.cat(proposal_list),
            object_feats=object_feats,
            attn_feats=attn_feats,
            # detach then use it in label assign
            detach_cls_score_list=[  # 每个张量都是候选目标的副本，与原始计算图无关 用英语标签分配
    ...
    def forward_train(self,
        ...
        num_proposals = proposal_boxes.size(1)  # 100
        imgs_whwh = imgs_whwh.repeat(1, num_proposals, 1) # [2 1 4] -> [2 100 4]
        all_stage_bbox_results = []
        proposal_list = [proposal_boxes[i] for i in range(len(proposal_boxes))]
        object_feats = proposal_features
        all_stage_loss = {}
        for stage in range(self.num_stages):  # num_stage = 6
            ...
            for i in range(num_imgs):
                normalize_bbox_ccwh = bbox_xyxy_to_cxcywh(proposal_list[i] /
                                                          imgs_whwh[i])
                assign_result = self.bbox_assigner[stage].assign(  # 标签分配 只有一个bbox会对应GT
                    normalize_bbox_ccwh, cls_pred_list[i], gt_bboxes[i],
                    gt_labels[i], img_metas[i])
                sampling_result = self.bbox_sampler[stage].sample(  # 对预测边界框进行采样 平衡正负样本数量
                    assign_result, proposal_list[i], gt_bboxes[i])  # 这里使用伪采样
                sampling_results.append(sampling_result)            # 因为使用了Focal loss
            bbox_targets = self.bbox_head[stage].get_targets(
                sampling_results, gt_bboxes, gt_labels, self.train_cfg[stage],
                True)
            cls_score = bbox_results['cls_score']  # [2 100 2]
            decode_bbox_pred = bbox_results['decode_bbox_pred'] # [200 4]
