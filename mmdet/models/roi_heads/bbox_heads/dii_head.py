...
class DIIHead(BBoxHead):
    ...
    def forward(self, roi_feat, proposal_feat):
        ...
        """
        N, num_proposals = proposal_feat.shape[:2]   # 2 100

        # Self attention  proposal_feat [2 100 256]   roi_feat [200 256 7 7]
        proposal_feat = proposal_feat.permute(1, 0, 2)  # [100 2 256]
        proposal_feat = self.attention_norm(self.attention(proposal_feat)) # attention注意力计算 attention_norm归一化（LN）
        attn_feats = proposal_feat.permute(1, 0, 2)  # [2 100 256]

        # instance interactive 通过动态卷积实现proposal_feat与roi_feat之间进行信息交互
        proposal_feat = attn_feats.reshape(-1, self.in_channels)  # [200 256] 两张图的proposal_bbox
        proposal_feat_iic = self.instance_interactive_conv(
            proposal_feat, roi_feat)  # 动态卷积得到的特征   proposal_feat[200 256] roi_feat[200 256 7 7]
        proposal_feat = proposal_feat + self.instance_interactive_conv_dropout(
            proposal_feat_iic)
        obj_feat = self.instance_interactive_conv_norm(proposal_feat)

        # FFN
        obj_feat = self.ffn_norm(self.ffn(obj_feat))  # [200 256]

        cls_feat = obj_feat
        reg_feat = obj_feat
        # 输入最后的分类（1个FC）与回归（3个FC）分支
        for cls_layer in self.cls_fcs:  # linear+ln+relu
            cls_feat = cls_layer(cls_feat)
        for reg_layer in self.reg_fcs:
            reg_feat = reg_layer(reg_feat)

        cls_score = self.fc_cls(cls_feat).view(   # [2 100 2]
            N, num_proposals, self.num_classes
            if self.loss_cls.use_sigmoid else self.num_classes + 1)
        bbox_delta = self.fc_reg(reg_feat).view(N, num_proposals, 4)  # [2 100 4]

        return cls_score, bbox_delta, obj_feat.view(  # [2 100 256]
            N, num_proposals, self.in_channels), attn_feats # [2 100 256]
    ...
    def loss(self,
        ...
        pos_inds = (labels >= 0) & (labels < bg_class_ind)  # [false true] 负样本为false 正样本为true
