...
class BBoxL1Cost:
    ...
    def __call__(self, bbox_pred, gt_bboxes):
        ...
        bbox_cost = torch.cdist(bbox_pred, gt_bboxes, p=1)  # 计算两个框之间的距离
...
class FocalLossCost:
    ...
    def _focal_loss_cost(self, cls_pred, gt_labels):
        ...
        """
        cls_pred = cls_pred.sigmoid()  # 这里计算了focal loss
        neg_cost = -(1 - cls_pred + self.eps).log() * (  # 负样本的损失函数
            1 - self.alpha) * cls_pred.pow(self.gamma)
        pos_cost = -(cls_pred + self.eps).log() * self.alpha * ( # 正样本的损失函数
            1 - cls_pred).pow(self.gamma)

        cls_cost = pos_cost[:, gt_labels] - neg_cost[:, gt_labels]  # [100 1]
