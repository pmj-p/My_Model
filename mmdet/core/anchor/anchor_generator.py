...
class AnchorGenerator:
    ...
    def gen_base_anchors(self):
        """Generate base anchors.
            先对单个位置(0, 0)生成base anchors 
        Returns:
            list(torch.Tensor): Base anchors of a feature grid in multiple \
                feature levels.
        """
        multi_level_base_anchors = []
        for i, base_size in enumerate(self.base_sizes):  # base_sizes = [8, 16, 32, 64, 128]
            center = None
            if self.centers is not None:
                center = self.centers[i]
            multi_level_base_anchors.append(
                self.gen_single_level_base_anchors(
                    base_size,          # base_sizes = [8, 16, 32, 64, 128]
                    scales=self.scales, # tensor([4.0000, 5.0397, 6.3496])
                    ratios=self.ratios, # tensor([0.5000, 1.0000, 2.0000])
                    center=center))
    ...
    def gen_single_level_base_anchors(self,
        ...
        """
        w = base_size  # base_sizes = [8, 16, 32, 64, 128]
        h = base_size  # base_sizes = [8, 16, 32, 64, 128]
        if center is None:
        ...
        # 计算高宽比例
        h_ratios = torch.sqrt(ratios) # tensor([0.5000, 1.0000, 2.0000])
        w_ratios = 1 / h_ratios
    ...
    def grid_priors(self, featmap_sizes, dtype=torch.float32, device='cuda'):
        ...
        return multi_level_anchors  # 0[136800, 4] 1[34200, 4] 2[8500,4] 3[2223, 4] 4[630, 4]
    ...
    def single_level_grid_priors(self,
        ...
        """
        # base_anchor 是对单个位置（0，0）生成base_anchor
        base_anchors = self.base_anchors[level_idx].to(device).to(dtype) 
        # base_anchors [9, 4]        
        feat_h, feat_w = featmap_size # 100 152
        stride_w, stride_h = self.strides[level_idx] # 8 8
        # First create Range with the default dtype, than convert to
        # target `dtype` for onnx exporting.
        shift_x = torch.arange(0, feat_w, device=device).to(dtype) * stride_w # [0,1,2...151]*8 -> [0,8,16...1208]
        shift_y = torch.arange(0, feat_h, device=device).to(dtype) * stride_h # [0,1,2...99]*8 -> [0,8,16...792]
        # [0,1,2...1208,0,...,1208,0,...,1208] 15200
        # [0,0,...0,8,8,...,8,16,16...16,...792,...,792,] 15200
        shift_xx, shift_yy = self._meshgrid(shift_x, shift_y) 
        shifts = torch.stack([shift_xx, shift_yy, shift_xx, shift_yy], dim=-1)
        # first feat_w elements correspond to the first row of shifts
        # add A anchors (1, A, 4) to K shifts (K, 1, 4) to get
        # shifted anchors (K, A, 4), reshape to (K*A, 4)
        # shift[152000 4] base_anchors[9,4]  不同尺度的anchor数量不同
        all_anchors = base_anchors[None, :, :] + shifts[:, None, :]
        all_anchors = all_anchors.view(-1, 4)  # all_anchors[15200*9, 4] -> [136800, 4]
    ...
    def valid_flags(self, featmap_sizes, pad_shape, device='cuda'):
        ...
        for i in range(self.num_levels):
            ...
            h, w = pad_shape[:2]
            valid_feat_h = min(int(np.ceil(h / anchor_stride[1])), feat_h) # 100
            valid_feat_w = min(int(np.ceil(w / anchor_stride[0])), feat_w) # 136
    ...
    def single_level_valid_flags(self,
        ...
        valid_x = torch.zeros(feat_w, dtype=torch.bool, device=device) # 152 [True]
        valid_y = torch.zeros(feat_h, dtype=torch.bool, device=device) # 100 [True]
        valid_x[:valid_w] = 1
        valid_y[:valid_h] = 1
        valid_xx, valid_yy = self._meshgrid(valid_x, valid_y)  # 15200 15200
