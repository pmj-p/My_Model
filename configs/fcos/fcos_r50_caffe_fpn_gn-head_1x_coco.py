...
model = dict(
    ...
    backbone=dict(
        ...
        norm_eval=True,
        style='caffe',  # 与pytorch的图片均值和方差不同 并且所有BN层参数不更新
        init_cfg=dict(  # 加载预训练模型
            type='Pretrained',
    ...
    neck=dict(
        ...
        add_extra_convs='on_output',    # 表示在FPN输出的最后一层特征层进行3*3的额外卷积
    ...
    test_cfg=dict(
        ...
        max_per_img=100))
img_norm_cfg = dict(
    mean=[0.112, 0.112, 0.112], std=[0.232, 0.232, 0.232], to_rgb=False)
train_pipeline = [
